...
for (i in 1:seq(2010,2019,1)){
  ...
  Frog_Mort_op_i <- rnorm(n=3, mean = mean, sd=1)
  Frog_Mort_op_i <- sort(Frog_Mort) # smaller values at the beginning
  Cars_op_i <- rnorm(n=3, mean = 50, sd = 8) 
  Cars_op_i <- sort(Cars)
# Assign cars to frog mortality in a dataset
  dat_op_i <- data.frame(Frog_Mort_op_i, Cars_op_i) # small cars have small frog mortality 
# add locations to it
  dat_op_i$Location <- rep("Opinicon Road",3)
  dat_op_i$Year <- rep("i",3)
  dat_op_i$Species <- c("x","y","z")
